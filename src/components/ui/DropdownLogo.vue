    ...
    <svg
      ...
      >
        <div class="flex flex-col font-mono text-xs gap-1">
          <p class="text-primary font-semibold px-2 my-2">
            {{ t('sidebar.navigation') }}
          </p>
          <DropdownMenuItem class="text-foreground  outline-none focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left" @click="focusOnDocuments()">
            {{ t('commandBar.focusProjects') }}
          </DropdownMenuItem>
          <DropdownMenuItem class="text-foreground  outline-none focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left" @click="focusOnTitle()">
            {{ t('commandBar.focusTitle') }}
          </DropdownMenuItem>
          <DropdownMenuItem class="text-foreground  outline-none focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left" @click="focusEditor()">
            {{ t('commandBar.focusEditor') }}
          </DropdownMenuItem>
          <DropdownMenuItem class="text-foreground  outline-none focus-visible:bg-primary/30 text-xs hover:bg-primary/20 px-2 py-2 text-left" @click="counter.showInfoModal = true">
            {{ t('sidebar.about') }}
          </DropdownMenuItem>
        </div>
      </DropdownMenuContent>
